...
class HdfsQuery:
    ...
    # ─────────────────────────────────────────────────────────
    #  MODULE 7 — EDA Thống kê khám phá dữ liệu
    # ─────────────────────────────────────────────────────────

    def text_length_stats(self, platform: Optional[str] = None) -> DataFrame:
        """
        Thống kê độ dài văn bản (min, max, mean, median, stddev) theo label và platform.
        Dùng để vẽ boxplot phân phối độ dài câu.

        Ví dụ
        -----
        q.text_length_stats().show()
        q.text_length_stats(platform="tiktok").show()
        """
        df = self._load_all()
        df = self._filter_platform(df, platform)
        df = df.withColumn("text_length", F.length(F.col("text")))
        return (
            df.groupBy("label", "platform")
            .agg(
                F.count("*").alias("count"),
                F.mean("text_length").alias("mean"),
                F.stddev("text_length").alias("stddev"),
                F.min("text_length").alias("min"),
                F.max("text_length").alias("max"),
                F.percentile_approx("text_length", 0.25).alias("q1"),
                F.percentile_approx("text_length", 0.50).alias("median"),
                F.percentile_approx("text_length", 0.75).alias("q3"),
            )
            .withColumn("label_name",
                F.when(F.col("label") == -1, "chưa gán nhãn")
                 .when(F.col("label") ==  0, "bình thường")
                 .when(F.col("label") ==  1, "offensive")
                 .when(F.col("label") ==  2, "hate speech")
                 .otherwise("không xác định"))
            .orderBy("label", "platform")
        )

    def topic_label_distribution(self, platform: Optional[str] = None) -> DataFrame:
        """
        Thống kê phân phối nhãn theo chủ đề (topic).
        Dùng để vẽ heatmap tỷ lệ nhãn theo topic.

        Ví dụ
        -----
        q.topic_label_distribution().show(30)
        """
        df = self._load_all()
        df = self._filter_platform(df, platform)
        return (
            df.groupBy("topic", "label")
    ...
    def source_label_distribution(self) -> DataFrame:
        """
        Thống kê phân phối nhãn theo nguồn (platform/source).
        Dùng để vẽ grouped bar chart so sánh nhãn giữa các nền tảng.

        Ví dụ
        -----
        q.source_label_distribution().show()
        """
        df = self._load_all()
        return (
            ...
            .withColumn("label_name",
                F.when(F.col("label") == -1, "chưa gán nhãn")
                 ...
                 .when(F.col("label") ==  2, "hate speech")
                 .otherwise("không xác định"))
            .orderBy("platform", "label")
        )

    def source_topic_distribution(self) -> DataFrame:
        """
        Thống kê số lượng bản ghi theo nguồn và chủ đề.
        Dùng để vẽ grouped bar chart source vs topic.

        Ví dụ
        -----
        q.source_topic_distribution().show(20)
        """
        df = self._load_all()
        return (
            df.groupBy("platform", "topic")
            .agg(F.count("*").alias("count"))
            .orderBy("platform", F.desc("count"))
        )

    # ─────────────────────────────────────────────────────────
    #  MODULE 8 — Truy cập dữ liệu đã qua xử lý
    # ─────────────────────────────────────────────────────────
